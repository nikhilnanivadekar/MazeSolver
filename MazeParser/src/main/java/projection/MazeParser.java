...
import projection.image.CoordinatePoint;
import projection.image.ImageWrapper;
import projection.image.Line;
import projection.image.MarkerColorRange;
...
import static projection.FeatureType.*;
...
//                        result.setEmpty(curRow, curCol);
    ...
    private ListIterable<MazeFeature> floodFill(ParsedMazeImage parsedMaze, int row, int column, int tag)
    {
        ...
        MazeFeature feature = parsedMaze.getFeatureAt(row, column);
        if (feature.isTagged()) { return boundary; }
        queue.add(feature);
